...
class SoccerEnv(gym.Env):
    ...
    def __init__(self, width=640, height=480):
        ...
        self.running = False  # Para controlar a thread de renderização
    ...
    def step(self, action):
        max_speed_per_step = 10.0  
        
        action = np.clip(action, -max_speed_per_step, max_speed_per_step)
        alpha = 0.3  
        self.goalkeeper_pos[0] += alpha * action[0]
    ...
    def _render_loop(self):
        """
        Loop contínuo de renderização para evitar bloqueios no treinamento.
        """
        while self.running:
